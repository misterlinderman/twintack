...
class Vite_App {
    ...
    public function __construct(
         ...
         */
        $this->is_hmr = defined( 'HMR_DEV' ) && 'woocommerce-wholesale-prices-premium' === HMR_DEV;
        ...
        $this->base_url = $this->is_hmr
            ? "$protocol//$host:$port/"
            : plugins_url( 'dist/', WWPP_PLUGIN_FILE );
        ...
        if ( ! empty( $l10n ) ) {
            if ( is_admin() ) {
                $this->dependencies = array_merge( $this->dependencies, array( 'wwpp-admin-l10n' ) );
            } else {
                $this->dependencies = array_merge( $this->dependencies, array( 'wwpp-frontend-l10n' ) );
            }
        }
    ...
    protected function parse_env() {
         ...
         */
        if ( ! file_exists( WWPP_PLUGIN_DIR_PATH . '.env' ) ) {
            return;
        }

        //phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
        $env = file_get_contents( WWPP_PLUGIN_DIR_PATH . '.env' );
        if ( $env ) {
            $env = preg_split( '/\\r\\n|\\r|\\n/', $env );
    ...
    public function global_l10n() {
        ...
        if ( is_admin() ) {
            if ( ! wp_style_is( 'wwpp-admin-style', 'registered' ) ) {
                wp_register_style( 'wwpp-admin-style', '', array(), get_plugin_data( WWPP_PLUGIN_FILE, false, false )['Version'] );
            }
            if ( ! wp_style_is( 'wwpp-admin-style' ) ) {
                wp_enqueue_style( 'wwpp-admin-style' );
            }
            if ( ! wp_script_is( 'wwpp-admin-l10n', 'registered' ) ) {
                wp_register_script( 'wwpp-admin-l10n', '', array( 'lodash' ), get_plugin_data( WWPP_PLUGIN_FILE, false, false )['Version'], false );
            }
            if ( ! wp_script_is( 'wwpp-admin-l10n' ) ) {
                wp_enqueue_script( 'wwpp-admin-l10n' );
                wp_localize_script( 'wwpp-admin-l10n', $this->l10n_name, array( 'pluginDirUrl' => plugins_url( '/', WWPP_PLUGIN_FILE ) ) );
            }
        } else {
            if ( ! wp_style_is( 'wwpp-frontend-style', 'registered' ) ) {
                wp_register_style( 'wwpp-frontend-style', '', array(), get_plugin_data( WWPP_PLUGIN_FILE, false, false )['Version'] );
            }
            if ( ! wp_style_is( 'wwpp-frontend-style' ) ) {
                wp_enqueue_style( 'wwpp-frontend-style' );
            }
            if ( ! wp_script_is( 'wwpp-frontend-l10n', 'registered' ) ) {
                wp_register_script( 'wwpp-frontend-l10n', '', array( 'lodash' ), get_plugin_data( WWPP_PLUGIN_FILE, false, false )['Version'], false );
            }
            if ( ! wp_script_is( 'wwpp-frontend-l10n' ) ) {
                wp_enqueue_script( 'wwpp-frontend-l10n' );
                wp_localize_script( 'wwpp-frontend-l10n', $this->l10n_name, array( 'pluginDirUrl' => plugins_url( '/', WWPP_PLUGIN_FILE ) ) );
            }
        }
    }
    ...
    protected function parse_manifest_file() {
         ...
         */
        $manifest_path = WWPP_PLUGIN_DIR_PATH . 'dist/.vite/manifest.json';
...
window.__vite_plugin_react_preamble_installed__ = true
            ...
            foreach ( $imports as $import ) {
                ...
                if ( ! empty( $import_styles ) ) {
                    foreach ( $import_styles as $import_style ) {
                        ...
                        wp_enqueue_style( "$this->script_handle-import-$sanitized_key", $css_url, array(), filemtime( WWPP_PLUGIN_FILE ) );
                    }
                }
            }
        }
    ...
    protected function enqueue_main_script() {
        ...
        /**************************************************************************
         * Enqueue/register main app script
         ***************************************************************************
         *
         ...
         */
        if ( null !== $this->l10n ) {
            $handle = is_admin() ? 'wwpp-admin-l10n' : 'wwpp-frontend-l10n';
        ...
        if ( in_array( 'wp-i18n', $this->dependencies, true ) ) {
            wp_set_script_translations(
                $this->script_handle,
                'woocommerce-wholesale-prices-premium',
                WWPP_PLUGIN_DIR_PATH . 'languages'
            );
        }
            ...
            if ( ! empty( $styles ) ) {
                foreach ( $styles as $style ) {
                    ...
                    if ( $this->register ) {
                        wp_register_style(
                            $this->script_handle,
                            plugins_url( "dist/$style", WWPP_PLUGIN_FILE ),
                            $this->style_dependencies,
                            filemtime( WWPP_PLUGIN_DIR_PATH . "dist/$style" )
                        );
                    } else {
                        wp_enqueue_style(
                            $this->script_handle,
        ...
        if ( $context instanceof \WP_Block_Editor_Context && 'my-post-type' === $context->post->post_type ) {
            $categories = array_merge(
                array(
                    array(
                        'slug'  => 'sample-blocks',
                        'title' => __( 'Sample Blocks', 'woocommerce-wholesale-prices-premium' ),
                    ),
                ),
                $categories
            );
    ...
    public function add_script_tag_attributes( $tag, $handle ) {
        ...
        if ( in_array( $handle, $handles, true ) ) {
            if ( $this->is_hmr ) {
                ...
                if ( apply_filters( 'wwpp_enable_subresource_integrity_check', ( defined( 'WWPP_ENABLE_SUBRESOURCE_INTEGRITY_CHECK' ) && WWPP_ENABLE_SUBRESOURCE_INTEGRITY_CHECK ) ) ) {
                    $integrity = sprintf(
                        'crossorigin="anonymous" integrity="%s" ',
                        $this->get_file_hash(
                            WWPP_PLUGIN_DIR_PATH . "dist/{$this->manifest[ $this->entry_file_path ]['file']}"
                        )
                    );
                }
                $tag = str_replace(
                ...
                if ( false !== strpos( $href, $style ) ) {
                    ...
                    if ( apply_filters( 'wwpp_enable_subresource_integrity_check', ! defined( 'WWPP_ENABLE_SUBRESOURCE_INTEGRITY_CHECK' ) || WWPP_ENABLE_SUBRESOURCE_INTEGRITY_CHECK ) ) {
                        $integrity = sprintf(
                            'crossorigin="anonymous" integrity="%s" ',
                            $this->get_file_hash(
                                WWPP_PLUGIN_DIR_PATH . "dist/$style"
                            )
                        );
                    }
