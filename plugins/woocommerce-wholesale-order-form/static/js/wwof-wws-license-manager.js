(function($, window){
    ...
    var software_key = 'wwof',
        $params      = wwof_license_manager_params;

    // This script is not yet ready to be publicly stored in the window.
	WWS_SLMW      = window.WWS_SLMW || {};
	WWOF_WWS_SLMW = window.WWOF_WWS_SLMW || {};
    ...
    WWOF_WWS_SLMW.Intersitial = ( function () {
        /**
    ...
    WWOF_WWS_SLMW.Reminder = ( function () {
        /**
     ...
     */
    $( function() {
        new WWOF_WWS_SLMW.Notice( $('.wws-license-notice') );
        new WWOF_WWS_SLMW.Intersitial( $('#wws-license-interstitial') );

        if ( window.WWS_SLMW.initialized_reminder.length < 1 ) {
            new WWOF_WWS_SLMW.Reminder();
		}
    });
