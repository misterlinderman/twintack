...
class Update_Manager extends Abstract_Update {
    ...
    private function ping_for_new_version( $force = false ) {
         ...
         * So that is why we still need to check the static json file, even if update info is still valid.
         *
         * @param array $option The request options.
         *
         * @since 3.0
         */
        $option = apply_filters(
            'wwof_plugin_new_version_ping_options',
            array(
        ...
        $response = wp_remote_retrieve_body(
            wp_remote_get(
            /**
             * Filter the plugin new version ping url.
             *
             * @param string $url The plugin new version ping url.
             *
             * @since 3.0
             */
                apply_filters( 'wwof_plugin_new_version_ping_url', WWOF_STATIC_PING_FILE ),
                $option
            )
        );
        ...
        if ( ! empty( $response ) ) {
            ...
            if ( ! empty( $response ) && property_exists( $response, 'version' ) ) {
                ...
                if ( ( ! $update_data && version_compare( $response->version, $installed_version, '>' ) ) ||
                    ...
                    if ( is_multisite() ) {
                        $this->fetch_software_product_update_data( get_site_option( WWOF_OPTION_LICENSE_EMAIL ), get_site_option( WWOF_OPTION_LICENSE_KEY ), home_url() );
                    } else {
                        $this->fetch_software_product_update_data( get_option( WWOF_OPTION_LICENSE_EMAIL ), get_option( WWOF_OPTION_LICENSE_KEY ), home_url() );
                    }
    ...
    private function fetch_software_product_update_data( $activation_email, $license_key, $site_url ) {
        ...
        $update_check_url = add_query_arg(
            ...
            /**
             * Filter the software product update data url.
             *
             * @param string $url The software product update data url.
             *
             * @since 3.0
             */
        ...
        /**
         * Filter the request options for fetching software product update data.
         *
         ...
         */
        $args = apply_filters(
            'wwof_software_product_update_data_options',
            array(
                'timeout' => 30,
        ...
        $result = wp_remote_retrieve_body( wp_remote_get( $update_check_url, $args ) );

        if ( ! empty( $result ) ) {

            $result = json_decode( $result );

            License_Manager::instance()->process_license_response( $result, 'update_data' );

            /**
             * Action hook to allow other plugins to do something after the software product update data is fetched.
             *
     ...
     * have updates ), we inject our plugin update data in, if any. It is saved on WWOF_UPDATE_DATA option. It is
     ...
     * @access public
     *
     * @return bool|object Filtered plugin updates data.
     */
    private function inject_plugin_update() {
        ...
        if ( $software_update_data ) {
            ...
            $update                       = new \stdClass();
            $update->name                 = 'WooCommerce Wholesale Order Form';
    ...
    public function inject_plugin_update_info( $result, $action, $args ) {
        ...
        if ( 'yes' === $license_activated && 'plugin_information' === $action && isset( $args->slug ) && 'woocommerce-wholesale-order-form' === $args->slug ) {
    ...
    public function maybe_update_plugin( $update, $plugin_data, $plugin_file ) {
        ...
        if ( WWOF_PLUGIN_BASE_NAME === $plugin_file ) {
