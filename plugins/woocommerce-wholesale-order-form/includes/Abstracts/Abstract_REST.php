...
abstract class Abstract_REST extends \WP_REST_Controller {
     ...
     * and `nonce` properties then call the parent method.
     *
     * @since 3.0
     * @return void
     */
    public function register_plugin_routes() {
         ...
         ***************************************************************************
         *
         * We set the default namespace to be the plugin's name and version if the
         * namespace property is not set.
         *
         * The namespace should be: wwof/v{major_version}
         * where {major_version} is the left-most major version of the plugin from
         * the main plugin file header.
         */
        $this->namespace = ! empty( $this->namespace )
            ? $this->namespace
            : 'wwof/v' . explode( '.', WWOF::get_current_plugin_version( false, false ) )[0];
        ...
        $this->rest_base = ! empty( $this->rest_base )
            ? $this->rest_base
            : strtolower( wp_basename( get_class( $this ) ) );
     ...
     *
     * @since 3.0
     * @return WP_Error|bool True if the request has access to update the item, false otherwise.
     */
    ...
    /**
     * Checks if a given request has access manage WooCommerce.
     *
     * @param WP_REST_Request $request Full data about the request.
     *
     * @since 3.0
     ...
     */
    public function can_manage_woocommerce( $request ) {
